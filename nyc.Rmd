...
demo_closed05_20002005_plot <- data_join %>% 
  ...
  theme_classic() +
   theme(legend.position = "none") +
  labs(subtitle = "Schools to Close",
...
demo_replacement05_20062015_plot <- data_join %>% 
  ...
  ylim(0, .6) +
  scale_color_discrete(name = "Race", labels = c("Asian", "Black", "Hispanic", "White")) +
  theme_classic() +
...
```{r Run the Regression, echo = FALSE}
...
regression <- data_join %>%
  subset(year >= 2006) %>% 
  mutate(total_grads_percent_of_cohort = total_grads_percent_of_cohort *100,
         
         # Create a logged variable of total high school enrollment 
         log10_total_hs_enrollment = log10(total_hs_enrollment)) %>% 
  lm(total_grads_percent_of_cohort ~ total_hs_enrollment + year + black_per + hispanic_per + white_per + asian_per + fl_or_frl_lunch, data = .)

# Reformatted output tables to not show scientific notation
...
stargazer(regression,
          ...
          header = FALSE,
          digits = 2,
          out = "tables/table1.txt")


# Remove transfer schools from analysis?
# Time fixed effects not done correctly yet
