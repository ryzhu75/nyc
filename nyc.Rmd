...
#Find the average graduation rate of schools 
...
close_school_2005_graduation_rates <- data_join %>% 
  ...
  summarize(percentile_25 = quantile(as.numeric(total_grads_percent_of_cohort), 
                                     probs = 0.25, 
                                     na.rm = TRUE),
            percentile_75 = quantile(as.numeric(total_grads_percent_of_cohort),
                                     probs = 0.75, 
                                     na.rm = TRUE),
...
closed_25_grad <- close_school_2005_graduation_rates %>% 
  pull(percentile_25)

closed_50_grad <- close_school_2005_graduation_rates %>% 
  pull(percentile_50)

closed_75_grad <- close_school_2005_graduation_rates %>% 
  pull(percentile_75)

# 25th percentile :24.8
# 50th percentile: 0.3885992
# 75th percentile: 0.351626

data_failing_open <- data_join %>% 
  group_by(dbn) %>% 
  nest() %>% 
  mutate("?2015" = map_lgl(data, ~ifelse(2015 %in% .[[2]], 
                        TRUE,
                        FALSE)))

#  filter(total_grads_percent_of_cohort <= closed_75_grad &
#         total_grads_percent_of_cohort >= closed_25_grad &
 #        !dbn %in% old_new_schools$old_dbn &
  #      !dbn %in% old_new_schools$new_dbn ) %>% 
#  select(dbn, school, total_grads_percent_of_cohort, enrnumg12) %>% 
 # group_by(school) %>% 
  #nest()

# What to do about schools that were closed later? Exclude them?

#data_failing_open_2015 <- data_failing_open %>% 
 # mutate("yesno_2015" = map_lgl(data, ~ifelse(2015 %in% .[[1]], 
            #       TRUE,
             #      FALSE))) %>% 
  #filter(yes#no_2015 == TRUE )
#data_failing_open_2015
