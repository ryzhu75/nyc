...
```{r OLS + Time Fixed Effects Regressions: grad percet ~ hs enrollment, echo = FALSE}
...
regression_tfe <- data_join %>%
  ...
  mutate(total_grads_percent_of_cohort = total_grads_percent_of_cohort *100,
         ...
         year = as.factor(year)) %>% 
  plm(total_grads_percent_of_cohort ~ total_hs_enrollment + black_per + hispanic_per + white_per + asian_per +     fl_or_frl_lunch, 
      data = .,
      index = "year",
