...
# Find the list of schools that had failing graduation rates but were not announced
...
data_join %>% 
  ...
  geom_col()+ 
  labs(title = "Number of High Schools with Enrollment ≤ 600 2000-2015",
       caption = "Sources: NYCDOE (2019), NYCDOE (2013), Research Alliance for NYC Schools (2016)") +
...
# Enrollment metrics of the schools that closed down

data_join %>% 
  
  # Filter for schools that closed down 
  filter(dbn %in% large_failing_schools) %>% 
  
  
  #large_failing_schools 
  
  # Filter for 2005 year
  filter(year == 2003) %>% 
  
  select(dbn, schnam, school, total_hs_enrollment) %>%
  summarize(percentile_25 = quantile(as.numeric(total_hs_enrollment),
                                     probs = 0.25,
                                     na.rm = TRUE),
            percentile_90 = quantile(as.numeric(total_hs_enrollment),
                                     probs = 0.90,
                                     na.rm = TRUE),
            percentile_50 = quantile(as.numeric(total_hs_enrollment),
                                     probs = 0.5,
                                     na.rm = TRUE))

# 25 Percnetile: 1307.5
# 50 Percentiile: 1603

  
# Filter for all large schools that were open in 2005 (>1300 enrollment)
failing_2005_large_list <- data_failing_open %>% 
  filter(enrollment_2005 > 1300) %>% 
  pull(dbn)
...
# Data for failing large schools in 2005 that were and are still open but shrank #(as defined as maintaining at less than 50% of its original enrollment in 2005
# and having enrollment at below 600)

# Data 
failing_2005_large_open_shrank <- data_failing_open %>% 
  filter(avail2015 == TRUE) %>%
  mutate("enrollment_2015" = map_dbl(data, ~filter(., year == 2015) %>% 
                                       pull(total_hs_enrollment))) %>% 
  filter(enrollment_2005 > 1300) %>% 
...
# (as defined as maintaining at more than than 50% of its original enrollment in 2005
# and having enrollment above 600)
...
## Data for all large schools (>1300 enrollment) schools, not yet announced for 
##  closure as of 2005,  with graduation rates in the middle
## 80th percentile of schools to be closed
...
replacement_schools_vs_all_failing %>% 
  ...
  ggplot(aes(x = year, y = avg_grad*100, shape = data_cat)) +
  geom_point(aes(shape = data_cat), size = 3) +
  geom_line() +
  labs(title = "Average Graduation Rates 2005-2015",
       y = "Average Graduation Rate (%)",
       x = "School Year (as of June)",
       caption = "Sources: NYCDOE (2019), NYCDOE (2013), Research Alliance for NYC Schools (2016) \nNotes: Values for the treated group in 2005 were omitted due to too few observations. \n           Values for the pretreatment group were omitted after 2008 due to too few observations.") +
...
replacement_schools_vs_all_failing %>% 
  ...
  # Add YoY Changes
  transform("control_yoy_change" = c(NA, failing_open_schools_in_2005[-1]/failing_open_schools_in_2005[-nrow(.)]-1),
            "treatment_yoy_change" =  c(NA, small_replacement_schools_opened_by_2005[-1]/small_replacement_schools_opened_by_2005[-nrow(.)]-1)) %>% 
  select(year, 
         failing_open_schools_in_2005, 
         control_yoy_change, 
         small_replacement_schools_opened_by_2005, 
         treatment_yoy_change, 
         announced_for_closure_in_2005) %>% 
  
# Create the gt table
  ungroup(year, data_cat)  %>% 
  gt() %>% 
  tab_header(title = "Average Graduation Rates 2005-2015") %>% 
  tab_spanner(label = "Graduation Rates", columns = 2:6) %>% 
  cols_label( "year" = "School Year (as of June)",
              "failing_open_schools_in_2005" = "Control Group",
              "control_yoy_change" = "YoYΔ (C)",
              "announced_for_closure_in_2005" = "Pretreatment",
              "treatment_yoy_change" = "YoYΔ (T)",
              "small_replacement_schools_opened_by_2005" = "Treatment") %>% 
  
  # Align center
  cols_align(align = "center", columns = c(1:6)) %>% 
  
  # Format decimals
  fmt_number(columns = 2:6, decimals = 2)  %>% 
  
  # Replace all NAs with "-" for readability
  fmt_missing(columns = TRUE, missing_text = "-") %>% 
  
  # Format percentage change columns
  fmt_percent(columns = c(3,5), decimals = 1) %>% 
    
  # Format all text to be times new roman and regular weight/style (except for YoY)
  tab_style(style = cell_text(font = "Times New Roman"), 
            locations = list(cells_column_labels(columns = c(1, 2, 4, 6)),
                          cells_body(columns = c(1, 2, 4, 6)), 
                          cells_title(), 
                          cells_column_spanners(spanners = "Graduation Rates")
                          ) 
            ) %>% 
  
  # Format YoY change columns to be times new roman in italics
  tab_style(style = cell_text(font = "Times New Roman", style = "italic"), 
            locations = list(cells_body(columns = c(3,5)),
                             cells_column_labels(columns = c(3,5))
                            )
            ) %>%
...
replacement_schools_grad_vs_failing %>% 
  ...
  # Add YoY Changes
  transform("control_open_yoy_change" = c(NA, large_and_operating[-1]/large_and_operating[-nrow(.)]-1),
            "control_shrank_yoy_change" = c(NA, large_and_shrank[-1]/large_and_shrank[-nrow(.)]-1),
            "control_closed_yoy_change" = c(NA, large_eventually_closed[-1]/large_eventually_closed[-nrow(.)]-1),
            "treatment_yoy_change" =  c(NA, small_replacement_schools_opened_by_2005[-1]/small_replacement_schools_opened_by_2005[-nrow(.)]-1)) %>% 
  select(year, 
         "large_and_operating",
         "control_open_yoy_change",
         "large_and_shrank",
         "control_shrank_yoy_change",
         "large_eventually_closed",
         "control_closed_yoy_change",
         "small_replacement_schools_opened_by_2005",
         "treatment_yoy_change",
         "announced_for_closure_in_2005") %>% 
  ...
  gt() %>% 
  tab_header(title = "Average Graduation Rates 2005-2015") %>% 
  tab_spanner(label = "Control", columns = 2:7) %>% 
  cols_label(year = "School Year (as of June)", 
             "large_and_operating" = "Open",
             "control_open_yoy_change" = "YoYΔ",
             "large_and_shrank" = "Shrank",
             "control_shrank_yoy_change" = "YoYΔ",
             "large_eventually_closed" = "Closed",
             "control_closed_yoy_change" = "YoYΔ",
             "small_replacement_schools_opened_by_2005" = "Treatment",
             "treatment_yoy_change" = "YoYΔ",
             "announced_for_closure_in_2005" = "Pretreatment") %>% 
  
  # Align center
  cols_align(align = "center", columns = 1:10) %>% 
  ...
  fmt_percent(columns = c(3,5,7,9), decimals = 1) %>% 
  
  # Format all text (except for YoY columns) to be Times New Roman and regular weight/style
  tab_style(style = cell_text(font = "Times New Roman"), 
            locations = list(cells_column_labels(columns = c(1,2,4,6,8,10)),
                          cells_body(columns = c(1,2,4,6,8,10)), 
                          cells_title(), 
                          cells_column_spanners(spanners = "Control")
                          ) 
            ) %>% 
  
  # Format YoY change columns to be Times New Roman in italics
  tab_style(style = cell_text(font = "Times New Roman", style = "italic"),
            locations = list(cells_body(columns = c(3,5,7,9)),
                             cells_column_labels(columns = c(3,5,7,9))
                             )
            ) %>% 
...
replacement_schools_grad_vs_failing %>% 
  ...
  ggplot(aes(x = year, y = avg_hs_enrollment, shape = data_cat)) +
  geom_point(aes(shape = data_cat), size = 3) +
  geom_line() +
  labs(title = "Average High School Enrollment 2005-2015",
...
grad_rates_nest %>% 
  ggplot(aes(x = year, y = percentile_50*100)) +
  geom_point() +
  geom_errorbar(aes(ymin = percentile_20*100, ymax = percentile_80*100)) +
  labs(title = "High School Graduation Rates 2005-2015",
       subtitle = "20, 50, 80 Percentiles",
       caption = "Sources: NYCDOE (2019), NYCDOE (2013)",
       y = "Graduation Rate (%)",
       x = "School Year (as of June)") +
...
grad_rates_regents_cohort %>% 
  ...
  geom_point() +
  geom_errorbar(aes(ymin = percentile_20*100, ymax = percentile_80*100)) +
  labs(title = "Regents Diploma Rate per Cohort 2005-2015",
       subtitle = "20, 50, 80 Percentiles",
       caption = "Sources: NYCDOE (2019), NYCDOE (2013)",
...
# Create a table for regents diploma

grad_rates_regents_cohort %>% 
  filter(year == 2005 |
         year == 2015) %>% 
  ungroup(cohort_year) %>% 
  select(year, percentile_20, percentile_80, percentile_50) %>% 
  pivot_longer(cols = starts_with("percentile_"), names_to = "percentile", values_to = "regents_rate") %>% 
  pivot_wider(names_from = "year", values_from = "regents_rate") %>% 
  mutate("percent_point_change" = ((.[[3]] - .[[2]])*100),
         "percent_change" = (.[[3]] - .[[2]])/.[[2]]*100,
         "percentile" = ifelse(percentile == "percentile_20", 
                               "20th",
  ...
  cols_label(percentile = "Percentile" ,
             percent_point_change = "PP",
             percent_change = "%") %>% 
  tab_spanner(label = "Regents Diploma Rate", columns = 2:3) %>% 
  tab_spanner(label = "Change", columns = 4:5) %>% 
  tab_header(title = "Regents Diploma Rate per Cohort 2005-2015") %>% 
  cols_align(align = "center", columns = TRUE) %>% 
  ...
  tab_style(style = cell_text(font = "Times New Roman"), 
            locations = list(cells_column_labels(columns = 1:5),
                          cells_body(), 
                          cells_title(), 
                          cells_column_spanners(spanners = c("Regents Diploma Rate", 
                                                             "Change"))
                          ) 
            ) %>% 
...
demo_closed05_20002005_plot <- data_join %>% 
  ...
  ggplot(aes(x = year, y = avg_per*100, shape = demographic)) +
  geom_line() +
  ...
  ylim(0, 60) +
  theme_classic() +
  labs(title = "Pre/Post Treatment Demographics",
       subtitle = "Pretreatment",
       y = "% of High School Enrollment") +
  theme(legend.position = "none",
...
demo_replacement05_20062015_plot <- data_join %>% 
  ...
  ylim(0, .6) +
  scale_shape_discrete(name = "", labels = c("Asian", "Black", "Hispanic", "White")) +
  theme_classic() +
...
# Combine above two plots to compare school and repelacement schools using patchwork package

demo_closed05_20002005_plot + 
  demo_replacement05_20062015_plot + 
  
  # Add plot annotations for source, since labelling the graphs individuallly would
  # cut off text
  plot_annotation(caption = "Sourcees: NYCDOE (2013), Research Alliance for NYC Schools (2016)", 
...
```{r Percent Poverty for Treatment and Control Groups, echo = FALSE}
# Graphing poverty rates for treatment/control/pretreatment groups from '06-'15

# Prepare the data to be graphed
poverty_exp <- data_join %>% 
  
  # % free and reduced lunch is a measure of poverty
  select(dbn, year, fl_or_frl_lunch) %>% 
  
  # We only have poverty rate data from 2006 onwards and our study doesn't
  # go until 2016
  filter(year >= 2006 & year <= 2015) %>% 
  filter(!dbn %in% large_failing_schools | year <= 2008) %>% 
  filter(dbn %in% failing_2005_large_list |
         dbn %in% replacement_schools |
         dbn %in% large_failing_schools) %>% 
  
  # New column that identifies category group of the observatiion
  mutate(data_cat = ifelse(dbn %in% large_failing_schools,
                           "pretreatment",
                              ifelse(dbn %in% replacement_schools,
                               "treatment",
                               "control"))) %>% 
  group_by(data_cat, year) %>% 
  nest() %>% 
  
  # Calculate the mean poverty rate for each group by year
  mutate(avg_frl_lunch = map(data, ~mean(.[[2]], na.rm = TRUE)))

# Plot the poverty rate from 2006-2015 by control/treatment group
poverty_exp %>% 
  ggplot(aes(x = year, y = as.double(avg_frl_lunch), shape = data_cat)) +
  geom_line() +
  geom_point(aes(shape = data_cat), size = 3) +
  
  scale_x_continuous(name = "School Year (as of June)",
                     breaks = c(2006, 2008, 2010, 2012, 2014)) +
  scale_y_continuous(name = "% Qualifying for Free and Reduced Lunch",
                     limits = c(50, 100)) +
  scale_shape_discrete(name = "", 
                       labels = c("Control", "Pretreatment", "Treatment")) +
  labs(title = "Average Poverty Rates 2006-2015",
       caption = "Sources: NYCDOE (2019), NYCDOE (2013) \nNotes: Values for the pretreatment group  after 2008 due to too few observations.") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0, face = "bold"),
        plot.title.position = "plot",
        plot.subtitle = element_text(hjust = 0),
...
```{r Poverty for Treatment and Control Groups Broken down into Closed Shrank Open, echo = FALSE}

## Graphing treatment and control poverty rates from '06-'15 
## // control group broken down further

# Prepare the data to be graphed
poverty_exp_detail <- data_join %>% 
  ...
  # New column that identifies category group of the observation
  mutate(data_cat = ifelse(dbn %in% large_failing_schools,
                           "pretreatment",
                              ifelse(dbn %in% replacement_schools,
                               "treatment",
...
poverty_exp_detail %>% 
  ...
  scale_x_continuous(name = "School Year (as of June)",
                     breaks = c(2006, 2008, 2010, 2012, 2014)) +
  scale_y_continuous(name = "% of Students Qualifying for Free and Reduced Lunch",
                     limits = c(40, 100)) +
  labs(title = "Average Poverty Rates 2006-2015") +
  theme_classic() +
  # Match economics paper aesthetic
  theme(plot.title = element_text(hjust = 0, face = "bold"),
        plot.title.position = "plot",
        plot.subtitle = element_text(hjust = 0),
...
data_join %>% 
  ...
  filter(lcgschtyp != "TRANSFER SCHOOL",
         lcgschtyp != "SPECIAL EDUCATION",
         lcgschtyp != "SUSPENSION CENTER") %>% 
...
stargazer(regression_tfe,
          ...
          title = "Regression Analysis: Total HS Enrollment and Graduation Rate",
          header = FALSE,
...
# Time fixed effects level-log regression using plm()
regression_tfe_level_log <- data_join %>%
  ungroup(year) %>% 
  ...
  mutate(total_grads_percent_of_cohort = total_grads_percent_of_cohort *100,
         ...
         #Convert year into a factor
         year = as.factor(year)) %>% 
  plm(total_grads_percent_of_cohort ~ log(total_hs_enrollment) + asian_per + black_per + hispanic_per + white_per + fl_or_frl_lunch, 
      data = .,
      index = "year",
...
# Check for Heteroskedasticity within the Time-Fixed Effects Model
# Reject Null Hypothesis of homoskedasticity
bptest(regression_tfe_level_log)

# Store robust standard errors into regression_tfe$robust  \\ Same as adding "r"
# at the end of regression in STATA
regression_tfe_level_log$robust <- coeftest(regression_tfe_level_log, vcovHC(regression_tfe_level_log, type = "HC1"))

# Plot time fixed effect regression
stargazer(regression_tfe_level_log,
          
          # Substitute in robust standard errors
          se = list(regression_tfe_level_log$robust[,2]),
          title = "Regression Analysis: Total HS Enrollment and Graduation Rate",
          header = FALSE,
          dep.var.labels= "Graduation Rate (% of Grade 9 Cohort)",
          
          covariate.labels = c("Total HS Enrollment (Logged)", 
                               "% Asian",
                               "% Black",
                               "% Hispanic", 
                               "% White",  
...
## OLS Regression + Time Fixed Effects Regression + Time Fixed Effects (level-log)
...
stargazer(regression_ols, regression_tfe, regression_tfe_level_log,
          
          # Replaced standard errors with robust standard errors
          se = list(regression_ols$robust[,2], regression_tfe$robust[,2]),
          title = "Regression Analysis: Total High School Enrollment on Graduation Rates",
          column.labels = c("OLS", "Time Fixed Effects", "Time Fixed Effects"),
          
          dep.var.labels= "Graduation Rate (% of Grade 9 Cohort)",
          
          covariate.labels = c("Total HS Enrollment", 
                               "Total HS Enrollment (log)",
                               "% Asian",
                               "% Black",
                               "% Hispanic", 
                               "% White",  
                               "% Free/Reduced Lunch"),
          # notes = "NYCDOE (2019), NYCDOE (2013), Research Alliance for NYC Schools (2016)",
          notes.align = "r",
          digits = 4,
...
data_join %>% 
  filter(year >= 2005,
         year < 2016) %>%
  filter(total_hs_enrollment > 400) %>% 
    # Remove special schools
  filter(lcgschtyp != "TRANSFER SCHOOL",
         lcgschtyp != "SPECIAL EDUCATION",
         lcgschtyp != "SUSPENSION CENTER") %>% 
  group_by(year) %>% 
  summarize(mean = mean(total_hs_enrollment))
